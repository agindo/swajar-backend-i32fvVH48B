...
##############################################
# Script untuk Create Pesertas from Certificates
# File ini akan memanggil API endpoint untuk
# create peserta dari certificate berdasarkan
# instansi, number, dan determine yang ditentukan
##############################################

# Configuration
API_URL="http://localhost:8071/api/certificates/create-pesertas"
LOG_FILE="./logs/create-pesertas-from-certificates.log"
...
# Print header
echo "========================================" | tee -a "$LOG_FILE"
echo "Create Pesertas from Certificates Script" | tee -a "$LOG_FILE"
echo "========================================" | tee -a "$LOG_FILE"
...
log_info "Starting peserta creation process at $(date '+%Y-%m-%d %H:%M:%S')"
echo ""
...
if [ "$HTTP_STATUS" -eq 200 ]; then
    ...
    log_info "Summary:"
    log_success "  ✓ Successfully created: $SUCCESS_COUNT pesertas"
    log_error "    ✗ Failed: $FAILED_COUNT"
    log_warning "    ⊗ Already exist: $ALREADY_EXIST_COUNT"
    log_info "  Total processed: $TOTAL certificates"
    
    echo ""
    log_success "Peserta creation process completed successfully!"
